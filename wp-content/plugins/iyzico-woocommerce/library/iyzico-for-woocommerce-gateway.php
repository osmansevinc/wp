...
class Iyzico_Checkout_For_WooCommerce_Gateway extends WC_Payment_Gateway {
    ...
    public function iyzico_payment_form($order_id,$payMethod = null) {
        ...
        if($payMethod != "pwi") {
            $iyzico                   = $formObjectGenerate->generateOption($customerCart,$getOrder,$apiKey,$this->affiliate);
        } else {
            $iyzico                   = $formObjectGenerate->generateOptionPwi($customerCart,$getOrder,$apiKey,$this->affiliate);
        }
        ...
        /* Hack Code */
        //$pkiString = str_replace(",cardUserKey=","",$pkiString);
    ...
    public function iyzico_response() {
        ...
        try {
            ...
            if (isset($requestResponse->installment) && !empty($requestResponse->installment) && $requestResponse->installment > 1) {
                ...
                $installment_fee    = $requestResponse->paidPrice - WC()->cart->get_totals()['total'];
